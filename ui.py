...
class SheetDialog(tk.Toplevel):
    ...
    def __init__(
        ...
        super().__init__(parent)
        self.title("Resistive Sheet")
        self.resizable(False, False)
        self.result: LayerConfig | None = None
        self.transient(parent)
        self.grab_set()

        self.rs_var = tk.StringVar(value=str(initial_resistance))

        frm = ttk.Frame(self, padding=10)
        frm.grid(row=0, column=0, sticky="nsew")

        ttk.Label(frm, text="Sheet resistance (\u03a9/sq)").grid(row=0, column=0, sticky="w")
        ttk.Entry(frm, textvariable=self.rs_var, width=14).grid(row=0, column=1, sticky="ew", padx=(8, 0))
...
class ImpedanceGui(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.layer_add_btn = ttk.Button(btns, text="Add Layer", width=12, command=self._add_layer)
        self.layer_add_btn.grid(row=0, column=0, pady=(0, 6))
        self.layer_add_sheet_btn = ttk.Button(btns, text="Add Sheet", width=12, command=self._add_sheet)
        self.layer_add_sheet_btn.grid(row=1, column=0, pady=6)
        self.layer_edit_btn = ttk.Button(btns, text="Edit", width=12, command=self._edit_layer)
        self.layer_edit_btn.grid(row=2, column=0, pady=6)
        self.layer_remove_btn = ttk.Button(btns, text="Remove", width=12, command=self._remove_layer)
        self.layer_remove_btn.grid(row=3, column=0, pady=6)
        self.layer_up_btn = ttk.Button(btns, text="Move Up", width=12, command=self._move_up)
        self.layer_up_btn.grid(row=4, column=0, pady=6)
        self.layer_down_btn = ttk.Button(btns, text="Move Down", width=12, command=self._move_down)
        self.layer_down_btn.grid(row=5, column=0, pady=6)
    ...
    def _draw_layer_preview(self) -> None:
        ...
        for i, (layer, layer_h) in enumerate(zip(self.layers, heights), start=1):
            ...
            if layer.is_sheet:
                self.layer_preview.create_line(
                    x0, (y + yn) * 0.5, x1, (y + yn) * 0.5,
                    fill=colors.get("accent", "#cc4444"),
                    width=2,
                    dash=(6, 3),
                )
        ...
        def worker() -> tuple[list[InverseCandidate], str, list[float], list[list[list[float]]]]:
            ...
            for mat_indices in product(*material_index_ranges):
                ...
                for i, layer in enumerate(layer_snapshot, start=1):
                    if layer.is_sheet:
                        candidate_tables_0.append(None)
                        candidate_tables_90.append(None)
                        continue
